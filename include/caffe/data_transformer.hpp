...
class DataTransformer {
 public:
  ...
  void LandmarkTransform(const Datum& datum, Blob<Dtype>* transformed_data,
                         Blob<Dtype>* transformed_label);
  
  void SegTransform(const Datum& datum, Blob<Dtype>* transformed_data,
                    Blob<Dtype>* transformed_label);
#endif  // USE_OPENCV
 ...
 protected:
  ...
  template<typename T>
  void DecodeString(const string& data, size_t idx, T* pf, size_t len) {
      memcpy(pf, const_cast<char*>(&data[idx]), len * sizeof(T));
  }
  
#ifdef USE_OPENCV
  struct LandmarkLabel {
    int num_objects;
    cv::Point2f center;
    cv::Mat landmarks;
  };
  
  void ReadLandmark(const std::string& data, int offset, int width,
